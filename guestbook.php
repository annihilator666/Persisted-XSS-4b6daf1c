...
$password = "ABC"; // PAS DEZE AAN ALS DAT NODIG IS
$db = "leaky_guest_book";
$conn;
try {
...
<div class="body-container">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $color = $_POST['color'];
        if (userIsAdmin($conn) === false && !($color === 'red')) {
            echo "<script> alert('not allowed') </script>";
        }
        if (filter_var($email, FILTER_VALIDATE_EMAIL) && userIsAdmin($conn)) {
            $conn->query("INSERT INTO `entries`(`email`, `color`, `admin`, `text`) 
                VALUES ('$email', '$color', '1', '$text');");
        } elseif (filter_var($email, FILTER_VALIDATE_EMAIL) && $color === 'red') {
            $conn->query("INSERT INTO `entries`(`email`, `color`, `admin`, `text`) 
                VALUES ('$email', '$color', '0', '$text');");
        }
    }
